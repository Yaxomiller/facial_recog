...
class CameraTests(unittest.TestCase):
    ...
    def test_open_camera_prefers_radxa_pipeline_before_direct_camera(self) -> None:
        ...
        self.assertEqual(camera.source_name, "Radxa GStreamer pipeline (/dev/video0)")
    ...
    def test_open_camera_raises_clear_error_when_all_attempts_fail(self) -> None:
        pipeline_capture = Mock()
        pipeline_capture.isOpened.return_value = False
        direct_capture = Mock()
        ...
        with (
            ...
            patch("src.camera.cv2.VideoCapture", side_effect=[pipeline_capture, direct_capture]),
        ):
